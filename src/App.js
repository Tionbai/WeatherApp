...
export default function App() {
  ...
  const updateCityState = (locationCity) => {
    currentLocation !== locationCity && setCurrentLocation(locationCity);
    city !== locationCity && setCity(locationCity);
  }
  ...
  const errorHandlerApiKey = () => {
    !process.env.REACT_APP_API_KEY && setError('You need an API key to make real calls.');
  }
    ...
    if (weather.cod) {
      switch (weather.cod) {
        case 200:
          ...
          break;
        default:
          setError(weather.cod + ' - ' + capitalizeErrorMsg(weather.message));
          break;
      }
    }
  }, [weather])
  ...
  return (
    <main className="App">
      <img className="App__image" src="https://images.unsplash.com/photo-1564750975191-0ed807751adf?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=634&q=80" alt={''}></img>
      <Header />
      {error &&
        <p className="App__error">{error}</p>}
      {!search ?
        <Form search={search} setSearch={setSearch} getCurrentLocation={getCurrentLocation} setCity={setCity} setError={setError} /> :
        <Forecast city={city} search={search} setSearch={setSearch} weather={weather} getWeather={getWeather} />
      }
    </main>
  );
}
